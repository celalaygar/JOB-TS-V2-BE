...
public class KanbanService {
    ...
    public Mono<PagedResult<ProjectTaskDto>> getFilteredBacklogTasks(
            KanbanTaskFilterRequestDto filterDto, int page, int size) {
    ...
    private Mono<PagedResult<ProjectTaskDto>> filterAndPaginateBacklogTasks(
            List<String> userProjectIds, KanbanTaskFilterRequestDto filterDto, int page, int size) {

        Query query = new Query();

        query.addCriteria(Criteria.where("backlog").exists(false));
        ...
        filterDto.getSprintId().ifPresent(sprintId ->
                query.addCriteria(Criteria.where("sprint.id").is(sprintId))
        );
